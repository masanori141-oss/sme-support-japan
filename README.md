...
- **`crawler/fetch_private_loans.py` の `LOAN_CONFIG` には、10分類に
  実データを登録済みです（計84件）。** 内訳はカードローン29・
  教育ローン15・自動車ローン13・リフォームローン8・住宅ローン7・
  ...
  1件のみ。三井住友信託銀行には有価証券担保ローンに相当する個人向け
  商品が見当たらず、2件目は要調査）。
  三井住友銀行の「フリーローン（標準）」は店頭表示金利（年6.625%、
  割引前）しか確認できず、他行の掲載値（優遇後金利）と比較条件が
  そろわないため目的型ローンには未登録（要調査）。
  京都銀行・北海道銀行・東邦銀行の3地方銀行は教育ローン・自動車ローン・
  リフォームローンへ、滋賀銀行・山陰合同銀行・伊予銀行は教育ローン・
  自動車ローンへ新たに対象を拡充した（いずれも公式サイトに明確な
  金利レンジ表記があり確認が容易だった）。これで対象12地方銀行の
  うち、教育ローンは10行（未登録：京葉銀行・静岡銀行）、自動車ローンは
  9行（未登録：常陽銀行・滋賀銀行・静岡銀行）をカバーした。
  ...
  - あおぞら銀行・GMOあおぞらネット銀行は個人向けカードローンを
    ...
    登録済み。
  - 信託銀行は個人向けカードローンを扱っていない可能性が高くカード
    ローンには未登録だが、投資不動産ローン（アパートローン）・証券
    担保ローンでは主要な取扱先になり得るため優先的に調査するとよい。
