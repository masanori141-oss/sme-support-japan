...
- **`crawler/fetch_private_loans.py` の `LOAN_CONFIG` には、10分類に
  実データを登録済みです（計89件）。** 内訳はカードローン29・
  教育ローン17・自動車ローン16・リフォームローン8・住宅ローン7・
  ...
  常陽銀行・静岡銀行は教育ローン・自動車ローンへ新たに対象を拡充した
  （いずれも公式サイトに明確な金利レンジ表記があり確認が容易
  だった）。これで対象12地方銀行のうち、教育ローン・自動車ローンとも
  12行中12行を登録済み（両分類とも全行カバー完了）。
