...
- **`crawler/fetch_private_loans.py` の `LOAN_CONFIG` は、まだ
  カードローン27件・教育ローン5件・自動車ローン5件・
  リフォームローン4件・住宅ローン5件（計46件）のみ実データを
  登録した段階です。**
  対象金融機関（メガバンク・信託銀行・新興銀行・政府系金融機関・
  地方銀行・消費者金融あわせて約38社）・対象融資分類11種のうち、
  不動産担保ローン・投資不動産ローン・証券担保ローン・目的型ローン・
  その他ローンの5分類はまだ未調査です（信託銀行は個人向け
  カードローンを扱っていない可能性が高く、カードローンには未登録。
  一方でみずほ信託銀行が独自のローン金利ページを持つことは確認済みで、
  要調査）。住宅ローンは物件価格・年収等で融資額が個別に決まるため、
  上限融資額を「要確認」のままにしている。`LOAN_CONFIG` に
  `LoanConfig` を1件ずつ追記していくことで拡充できます（Claudeに
  「◯◯銀行の不動産担保ローンを調べて LOAN_CONFIG に追加して」と
  頼めば対応できます）。
