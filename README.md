...
- **`crawler/fetch_private_loans.py` の `LOAN_CONFIG` には、10分類に
  実データを登録済みです（計121件）。** 内訳はカードローン31・
  教育ローン20・自動車ローン18・住宅ローン21・リフォームローン16・
  不動産担保ローン5・投資不動産ローン5・目的型ローン4・
  証券担保ローン1（その他ローンは該当商品なしのため0件）。
  住宅ローンは対象12地方銀行のうち11行（未登録は京都銀行のみ、
  公式サイトの優遇後金利がJS描画で自動取得できなかったため要調査）、
  リフォームローンは12行中12行全行をカバーした。
