...
- **`crawler/fetch_private_loans.py` の `LOAN_CONFIG` は、まだ
  カードローン27件・教育ローン5件・自動車ローン5件（計37件）のみ
  実データを登録した段階です。**
  ...
  その他ローン）のうち、リフォームローン以降の6分類はまだ未調査です
  （信託銀行は個人向けカードローンを扱っていない可能性が高く、
  カードローンには未登録）。`LOAN_CONFIG` に `LoanConfig` を
  1件ずつ追記していくことで拡充できます（Claudeに「◯◯銀行の
  リフォームローンを調べて LOAN_CONFIG に追加して」と頼めば
  対応できます）。
