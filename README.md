...
- **`crawler/fetch_private_loans.py` の `LOAN_CONFIG` には、10分類に
  実データを登録済みです（計93件）。** 内訳はカードローン30・
  教育ローン17・自動車ローン16・リフォームローン8・住宅ローン10・
  不動産担保ローン4・投資不動産ローン4・目的型ローン3・
  証券担保ローン1（その他ローンは該当商品なしのため0件）。
  東邦銀行の教育ローンのみ限度額が未確認（金利は確認済み）。
  対象金融機関は約38社中まだ一部
  （メガバンク・地方銀行の主要行、新興銀行の大半、信託銀行2行、
  消費者金融、政府系金融機関1行）にとどまっており、分類ごとの
  登録金融機関数にも大きな差があります（証券担保ローンは楽天銀行の
  1件のみ）。証券担保ローンについては、みずほ信託銀行・三井住友信託
