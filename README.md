...
- **`crawler/fetch_private_loans.py` の `LOAN_CONFIG` には、11分類
  すべてに最低1件は実データを登録済みです（計69件）。** 内訳は
  カードローン29・教育ローン9・自動車ローン8・住宅ローン7・
  リフォームローン6・投資不動産ローン4・不動産担保ローン3・
  証券担保ローン1・目的型ローン2（その他ローンは該当商品なしのため
  0件）。対象金融機関は約38社中まだ一部（メガバンク・地方銀行の主要行、
  新興銀行の大半、信託銀行2行、消費者金融、政府系金融機関1行）に
  とどまっており、分類ごとの登録金融機関数にも大きな差があります
  （証券担保ローンは楽天銀行の1件のみ。三井住友信託銀行には有価証券
  担保ローンに相当する個人向け商品が見当たらず、2件目は要調査）。
