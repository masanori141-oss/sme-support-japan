...
- **`crawler/fetch_private_loans.py` の `LOAN_CONFIG` には、11分類
  すべてに最低1件は実データを登録済みです（計55件）。** 内訳は
  カードローン27・教育ローン5・自動車ローン5・リフォームローン4・
  住宅ローン5・不動産担保ローン3・投資不動産ローン3・証券担保ローン1・
  目的型ローン2（その他ローンは該当商品なしのため0件）。ただし
  対象金融機関は約38社中まだ一部（メガバンク・地方銀行の主要行、
  新興銀行の一部、信託銀行2行、消費者金融）にとどまっており、
  分類ごとの登録金融機関数にも大きな差があります（証券担保ローンは
  楽天銀行の1件のみ等）。信託銀行は個人向けカードローンを扱って
  いない可能性が高くカードローンには未登録だが、投資不動産ローン
  （アパートローン）・証券担保ローンでは主要な取扱先になり得るため
  優先的に調査するとよい。住宅ローンは物件価格・年収等で融資額が
  個別に決まるため、上限融資額を「要確認」のままにしている。
  一部の商品（投資不動産ローンの三井住友信託銀行・三井住友銀行分）は
  金利が非公開または未確認のため `confirmed=False` としており、
  総合台帳の金利順ソートには乗らない。`LOAN_CONFIG` に
  `LoanConfig` を1件ずつ追記していくことで拡充できます（Claudeに
  「◯◯銀行のその他ローンを調べて LOAN_CONFIG に追加して」と
  頼めば対応できます）。
