...
- **`crawler/fetch_private_loans.py` の `LOAN_CONFIG` には、11分類
  すべてに最低1件は実データを登録済みです（計63件）。** 内訳は
  カードローン29・教育ローン8・自動車ローン5・リフォームローン4・
  住宅ローン7・投資不動産ローン4・不動産担保ローン3・証券担保ローン1・
  目的型ローン2（その他ローンは該当商品なしのため0件）。対象金融機関は
  約38社中まだ一部（メガバンク・地方銀行の主要行、新興銀行の大半、
  信託銀行2行、消費者金融、政府系金融機関1行）にとどまっており、
